...
test.describe('超级管理员社区管理测试', () => {
  ...
  test('验证默认社区保护机制', async ({ page }) => {
    ...
    // 检查是否有错误提示
    const hasError = pageTextAfterClick.includes('无法停用') || 
    ...
    if (hasError) {
      console.log('✅ 检测到错误提示:', pageTextAfterClick.match(/无法停用|默认社区|不允许/)?.[0]);
    }
  ...
  test('创建新社区', async ({ page }) => {
    ...
    console.log(`测试数据: 社区名称="${newCommunityName}", 位置="${newCommunityLocation}"`);
