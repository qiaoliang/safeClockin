...
function isValidURL(url) {
    ...
    try {
        ...
        // 对于相对URL，添加基础URL进行验证
        const testUrl = trimmedUrl.startsWith("http")
            ? trimmedUrl
            : `http://example.com${
        ...
        if (parsedUrl.hostname) {
            ...
            const internalIPPattern =
                /^(localhost|127\.0\.0\.1|10\.|172\.1[6-9]\.|172\.2[0-9]\.|172\.3[0-1]\.|192\.168\.|169\.254\.)/;
            if (internalIPPattern.test(parsedUrl.hostname)) {
                console.warn(
                    "⚠️ Layer 1警告: URL包含内网地址",
                    parsedUrl.hostname
                );
            }
        }
...
describe("URL验证函数测试", () => {
    ...
    it("应该验证有效的相对URL", () => {
        expect(isValidURL("/api/community_checkin/rules?community_id=3")).toBe(
            true
        );
    ...
    it("应该清理微信小程序环境后缀", () => {
        const urlWithSuffix =
            "/api/test (env: macOS,mp,1.06.2504060; lib: 3.11.3)";
