<template>
  <uni-section class="test-container" title="uni-ui 组件测试页面" type="line">
    <!-- 最简单的 uni-ui 组件展示 -->
    <uni-card title="快速开始" extra="基础组件" thumbnail="https://picsum.photos/100/100?random=1">
...
<uni-card>
    ...
    <!-- 徽章组件展示 -->
    <uni-card title="Badge 徽章组件" extra="多种样式" thumbnail="https://picsum.photos/100/100?random=2">
      <uni-row :gutter="20">
        <uni-col :span="6">
    ...
    <!-- 卡片组件展示 -->
    <uni-card title="Card 卡片组件" extra="交互示例" thumbnail="https://picsum.photos/100/100?random=3">
      <text>这是卡片的内容区域，可以放置任意内容。</text>
      <template v-slot:actions>
        <uni-row :gutter="20">
          <uni-col :span="6">
            <button size="mini" type="primary" @click="handleClick">点击测试</button>
          </uni-col>
          <uni-col :span="6">
    ...
    <!-- 列表组件展示 -->
    <uni-card title="List 列表组件" extra="数据展示" thumbnail="https://picsum.photos/100/100?random=4">
      <uni-list>
    ...
    <!-- 按钮组件展示 -->
    <uni-card title="Button 按钮组件" extra="操作按钮" thumbnail="https://picsum.photos/100/100?random=5">
      <uni-row :gutter="20">
        <uni-col :span="8">
    ...
    <!-- 分组组件展示 -->
    <uni-card title="Section 分组组件" extra="内容分组" thumbnail="https://picsum.photos/100/100?random=6">
      <uni-section title="子分组标题" type="line">
        <text>这是分组内容，可以包含任意组件。</text>
        <view class="section-content">
          <uni-tag text="标签1" type="primary" />
          <uni-tag text="标签2" type="success" />
          <uni-tag text="标签3" type="warning" />
        </view>
      </uni-section>
    </uni-card>
    
    <!-- 更多组件展示 -->
    <uni-card title="更多 uni-ui 组件" extra="扩展功能" thumbnail="https://picsum.photos/100/100?random=7">
      <uni-row :gutter="20">
        <uni-col :span="12">
          <uni-section title="评分组件" type="dot">
            <uni-rate v-model="rate" />
          </uni-section>
        </uni-col>
        <uni-col :span="12">
          <uni-section title="数字输入框" type="dot">
            <uni-number-box v-model="number" :min="1" :max="10" />
          </uni-section>
        </uni-col>
      </uni-row>
      
      <uni-section title="步骤条" type="dot">
        <uni-steps :options="steps" :active="activeStep" />
      </uni-section>
      
      <uni-section title="弹出层" type="dot">
        <button @click="showPopup = true">显示弹出层</button>
        <uni-popup ref="popup" :show="showPopup" type="center" @change="change">
